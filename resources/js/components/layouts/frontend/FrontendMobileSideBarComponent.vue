            ...
            <div class="px-3 pb-10">
                ...
                <div v-if="setting.site_language_switch === enums.activityEnum.ENABLE">
                    ...
                    <ul id="mobile-language-colspan" class="w-full flex flex-col gap-2 mb-3 transition-all duration-500 h-0 overflow-hidden">
                        <li v-for="(LoopLanguage, index) in languages" :key="index" @click.prevent="changeLanguage(LoopLanguage.id, LoopLanguage.code)" class="flex items-center gap-3 px-2.5 py-2 rounded-lg w-full cursor-pointer bg-slate-100">
